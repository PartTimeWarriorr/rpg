...
use std::{env, fmt, fs::File, hash::RandomState, io::BufReader, path, sync::Arc}; 
...
impl fmt::Debug for MainState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("MainState") 
         ...
         .field("friendly_party", &self.friendly_party)
         .field("all_enemies", &self.all_enemies)
         .finish()
    }
}
